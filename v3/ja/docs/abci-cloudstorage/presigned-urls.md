...
## 利用方法
署名付きURLは以下のコマンドで作成可能です。`<s3 object path>`には対象とするオブジェクトを指定し、`[--expires-in <expiration time (seconds)>]` には有効期限を秒単位で指定します。有効期限は最大7日間まで設定可能です。`[--expires-in]`オプションを省略した場合は、3600 秒（1時間）がデフォルト値として使用されます。
```
