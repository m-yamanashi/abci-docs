...
Specify the target object in `<s3 object path>`, and set the publication period in seconds in `[--expires-in <expiration time (seconds)>]`. The expiration time can be set as high as 7 days. If the `[--expires-in]` option is omitted, the default value of 3600 seconds (1 hour) is used.
```
